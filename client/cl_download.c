...
void CL_RequestNextDownload (void)
{
	...
	// Knightmare- clear failed download list
	if (precache_check == CS_MODELS)
		CL_InitFailedDownloadList ();

	// Knightmare- skip to loading map if downloading disabled or on local server
...
//ZOID
	...
	{
		if ( Com_ServerState() )	// Knightmare- if on local server, skip checking textures
